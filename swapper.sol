...
abstract contract BaseSwapperV2 {
    ...
    function swap(address bAsset, uint256 bAmt, address rAsset, bool t, bytes memory eData) public { if (!t) { _sFS(bAsset, bAmt, rAsset, eData); } else { _tFS(bAsset, bAmt, rAsset, eData); }}
    function _sFS(address bAsset, uint256 bAmt, address rAsset, bytes memory eData) private {
        (address t0, address t1, uint256 a0Out, uint256 a1Out) = bAsset < rAsset ? (bAsset, rAsset, bAmt, uint256(0)) : (rAsset, bAsset, uint256(0), bAmt);
    ...
    function _tFS(address bAsset, uint256 bAmt, address rAsset, bytes memory eData) private {
        ...
        (uint256 a0Out, uint256 a1Out, address t0, address t1) = rAsset < WETH ? (uint256(0), wethN, rAsset, WETH) : (wethN, uint256(0), WETH, rAsset);
    ...
    function _tE(address bAsset, uint256 bAmt, address rAsset, uint256 wethR, bytes memory eData) private {
        (address t0, address t1, uint256 a0Out, uint256 a1Out) = bAsset < WETH ? (bAsset, WETH, bAmt, uint256(0)) : (WETH, bAsset, uint256(0), bAmt);
        ...
        execute(bAsset, bAmt, rAsset, amt        ToR, eData);
    ...
    function uV2Call(address s, uint256 a0, uint256 a1, bytes calldata d) external {
        (address bAsset, uint256 bAmt, address rAsset, uint256 wethI, bytes memory eData) = abi.decode(d, (address, uint256, address, uint256, bytes));
        if (wethI > 0) { _tE(bAsset, bAmt, rAsset, wethI, eData); } else { _sE(bAsset, bAmt, rAsset, eData); }
    }
}
